...
PackageCount=`ls waterfox*.tar.bz2 | awk 'END { print NR }'`

# Make package name editable in single place in the event of file naming change.
Package=$Dir/waterfox-*.tar.bz2
...
select yn in "Install" "Uninstall" "Quit"; do
    case $yn in
        ...
        if [ -f $Dir/waterfox*.tar.bz2 ]; then
            ...
            # Unpack waterfox in to apps directory, Remove existing waterfox folder.
...
END
            ...
            sudo ln -sf $InstallDirectory/waterfox/browser/icons/mozicon128.png /usr/share/icons/hicolor/128x128/apps/waterfox.png
